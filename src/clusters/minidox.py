...
class MinidoxCluster(DefaultCluster):
    ...
    def thumborigin(self):
        ...
        origin[1] = origin[1] - .4 * (self.settings["trackball_Usize"] - 1) * self.settings["sa_length"]
        return origin
    ...
    def ml_place(self, shape):
        ...
        shape = self.helper.translate(shape, self.thumborigin())
        shape = self.helper.translate(shape, [-53, -26, -12])
        return shape
    ...
    def screw_positions(self):
        ...
        position[1] = position[1] - .4 * (self.minidox_Usize - 1) * self.settings["sa_length"]
        position[2] = 0
