...
class TrackballOrbyl(DefaultCluster):
    ...
    def mr_place(self, shape):
        ...
        shape = self.helper.translate(shape, (t_off[0], t_off[1] + self.key_diameter/2, t_off[2]))
        shape = self.helper.rotate(shape, [0,0,-130])
        shape = self.track_place(shape)
    ...
    def thumb(self, side="right"):
        ...
        shape = self.helper.union([shape, self.thumb_fx_layout(plate.double_plate(self.settings, self.helper))])
        shape = self.helper.union([shape, self.thumb_1x_layout(plate.single_plate(self.settings, self.helper, side=side))])
    ...
    def walls(self, side="right"):
        ...
        shape = self.helper.union([shape, self.helper.wall_brace(
            self.mr_place, .5, 1, self.connector.web_post_tr(),
